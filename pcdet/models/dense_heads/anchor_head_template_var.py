...
class AnchorHeadTemplateVAR(AnchorHeadTemplate):
    def get_box_reg_layer_loss(self):
        ...
        box_preds = box_preds.view(batch_size, -1,
                                   ...
                                   box_preds.shape[-1])
        var_preds = box_preds.view(batch_size, -1,
                                   box_preds.shape[-1] // self.num_anchors_per_location if not self.use_multihead else
                                   box_preds.shape[-1])
        # sin(a - b) = sinacosb-cosasinb
        box_preds_sin, reg_targets_sin = self.add_sin_difference(box_preds, box_reg_targets)
        loc_loss_src, l1_loss_src, var_loss_src, calib_loss_src = self.reg_loss_func(box_preds_sin, var_preds, reg_targets_sin, weights=reg_weights)  # [N, M]
